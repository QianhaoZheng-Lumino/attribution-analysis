...
2. **必填 H3**与骨架相同。只有两处可替换：`### 2b 定责 → **{结论}**`、`### 3a 配置（checklist **{N}/14** {✅/⚠️}，窗口 {w}）`。`### 2c 下钻` 无后缀。HTML 上定责标题去掉 `2b`，箭头右边原样照抄，不翻译、不补供应商名。**2b=C/Dida 填 4_/6_ 单维；2b=S/CS 填 5_/7_/03 复合维**，只留一条路径。
...
4. **表头逐字**。3a = **7 列 14 行**（`#` / `Level` / `n` / `操作(枚举)` / `作用域` / `Δmargin / remark` / `信号`），**不写**来源 lite 文件。§4 责任修正四行 = `Phase 2b 初判` / `修正后主因` / `并列` / `非主因（已排除）`。证据对照表七行不变。**修正后主因**只约束综合判断表那一格，定责标题不套这个格式，箭头右边照抄。这一格写成 `S（短名）` 或 `S（短名，细节）`，例如 `S（HotelTrader 全网，美国 / 温德姆）`。代号只许 `S`、`CS`、`C`、`Dida`、`C/Dida`，前面可加「倾向」，括号用中文。短名里不要逗号，细节放在第一个逗号后面。页眉「主因」只显示短名；括号里没有逗号时，页眉会把整句放进去。页眉「异动」只取综合评分开头的 `N/100`，后面的构成说明留在格子里，不会再整段进页眉。不要写成 `C：渠道停查` 或 `Dida · CBD 加价`，不要在括号外面再加字，不要一格写两条。第二条放进「并列」。证据未验时写成 `倾向 C/Dida（宽口径，配置与查验订未验）`，括号外不要写「保留」或冒号后的长句。格式不对则检查失败，不生成 HTML。
5. **先复制骨架填空，再按触发条件删整节。** 在线：日均写成 `22.07h` 或 `22.07小时` 才保留 `#### 在线时长`；没有可比日均就删掉这一整节。证据对照表仍留「在线时长」一行，写「未验」或「未查」。准确率：每案 `01-total`。探测行能读出 **Δpp** 时，绝对值 ≥5 才写「启动后下钻」，小于 5 则删掉该节，也不留空表。探测失败写成 `MCP 500 · 未验`，并且不要写「启动后下钻」。格子里的「未启动」不能代替 Δpp。外部事件：未查或未取则整节不写，证据对照表仍留「D 外部」一行；查过并有节日、命中或 0 行，整节留下。
...
- [ ] 2. 复制 04-report-skeleton.md，只填空，再按上面第 5 条删掉未触发的整节；`python scripts/check-report-skeleton.py` 通过后，再 `python scripts/render-report-html.py`。失败则不写 HTML。对话只回该文件的 Executive Summary 和 HTML 路径
