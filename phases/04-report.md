...
4. **表头逐字**。3a = **7 列 14 行**（`#` / `Level` / `n` / `操作(枚举)` / `作用域` / `Δmargin / remark` / `信号`），**不写**来源 lite 文件。§4 责任修正四行 = `Phase 2b 初判` / `修正后主因` / `并列` / `非主因（已排除）`。证据对照表七行不变。**修正后主因**只约束综合判断表那一格，定责标题不套这个格式，箭头右边照抄。这一格写成 `S（短名）` 或 `S（短名，细节）`，例如 `S（HotelTrader 全网，美国 / 温德姆）`。代号只许 `S`、`CS`、`C`、`Dida`、`C/Dida`，前面可加「倾向」，括号用中文。短名里不要逗号，细节放在第一个逗号后面。页眉「主因」只显示短名；括号里没有逗号时，页眉会把整句放进去。不要写成 `C：渠道停查` 或 `Dida · CBD 加价`，不要在括号外面再加字，不要一格写两条。第二条放进「并列」。格式不对则检查失败，不生成 HTML。
5. **在线时长、准确率、外部事件先探测再按触发填。** 在线：日均写成 `22.07h` 或 `22.07小时` 才算有数，才写「在线时长」整节；未触发或日均为空则整节不写，证据对照表仍留「在线时长」一行。准确率：每案 `01-total`；以探测行里的 **Δpp 绝对值**为准，≥5 才写「启动后下钻」，小于 5 则不写该节，也不留空表。格子里留下菜单原文「未启动」不能代替这个数字。外部事件：未查或未取则整节不写，证据对照表仍留「D 外部」一行；查过并有节日、命中或 0 行，整节留下。
...
- [ ] 2. 复制 04-report-skeleton.md，只填空；`python scripts/check-report-skeleton.py` 通过后，再 `python scripts/render-report-html.py`。失败则不写 HTML。对话只回该文件的 Executive Summary 和 HTML 路径
